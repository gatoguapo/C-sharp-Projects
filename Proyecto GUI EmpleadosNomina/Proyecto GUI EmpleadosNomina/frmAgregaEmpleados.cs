...
namespace Proyecto_GUI_EmpleadosNomina
{
    public partial class frmAgregaEmpleados : Form
    {
        ...
        private void frmAgregaEmpleados_Load(object sender, EventArgs e)
        {
            ...
            {
                txtClaveEmp.Text = listEmp.Last().pClave+2.ToString(); //En caso de que ya exista una, regresa el valor de la ultima clave registrada.
            }
            rbMasculino.Checked = true; // <= Selecciona el rbMasculino por defecto.
            List<Departamento> listDep = manejaDep.getDepartamentos();
            listDep.ForEach(dep => cmbClaveDepto.Items.Add(dep.pClaveDepto));
        }
        ...
        private bool validaCMB()
        {
            ...
            {
                errorP.SetError(cmbClaveDepto, "Debes ingresar una carrera primero");
                return false;
            }
            return true;
        }
